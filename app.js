...
function searchResultTileHTML(item, idx, newIds) {
  const priceLine = [
    item.price != null ? `${esc(item.currency || "")} ${esc(item.price)}`.trim() : "Price n/a",
    item.condition
  ].filter(Boolean).join(" · ");

  return `
    <div class="card-tile ${newIds && newIds.has(item.id) ? "is-hit" : ""}" data-item-id="${esc(item.id)}" data-url="${esc(item.itemWebUrl || "")}" title="Item ID: ${esc(item.id || "")}">
      <div class="card-thumb">${item.imageUrl ? `<img src="${esc(item.imageUrl)}" alt="" />` : "🎴"}</div>
      <div class="card-info">
        <div class="player-name">${esc(item.title)}</div>
        <div class="card-meta">${priceLine}</div>
        <div class="badge-row">
          ${newIds && newIds.has(item.id) ? `<span class="badge badge-gold">New</span>` : ""}
          ${item.seller ? `<span class="badge">Seller: ${esc(item.seller)}</span>` : ""}
          <button class="badge" data-action="view" data-url="${esc(item.itemWebUrl)}" style="cursor:pointer">View listing ↗</button>
          <button class="badge" data-action="identify" data-idx="${idx}" style="cursor:pointer">✨ Identify</button>
          <button class="badge" data-action="want" data-idx="${idx}" style="cursor:pointer">+ Wishlist</button>
        </div>
      </div>
    </div>`;
}
...
function wireResultButtons(container, results) {
  container.querySelectorAll(".card-tile[data-item-id]").forEach((tile) => {
...
function renderSearchResults(results, query, opts = {}) {
  ...
  el.innerHTML = `<div class="section-title" style="margin-top:18px">${results.length} result${results.length > 1 ? "s" : ""}${newIds ? ` · ${newIds.size} new since last check` : ""}</div>` +
    results.map((item, idx) => searchResultTileHTML(item, idx, newIds)).join("");

  wireResultButtons(el, results);
}
...
async function renderSavedSearches() {
  ...
  if (searches.length === 0) {
    el.innerHTML = `<div class="empty-state" style="padding:20px 10px">
      <p style="margin:0">No saved searches yet. Search in Scout, then tap ☆ Save to keep track of it here.</p>
    </div>`;
    return;
  }
